...
- Бот отвечает только пользователям, которые присоединились по реферальной ссылке администратора.
- ID администратора указывается в переменной окружения `ADMIN`; он может получить ссылку командой `/createreferral`.
